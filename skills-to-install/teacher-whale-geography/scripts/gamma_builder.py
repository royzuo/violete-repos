...
The script posts to Gamma's public Generations API, polls until completion,
and prints parseable output for downstream automation.
"""
...
def request_json(method, url, api_key, payload=None):
    ...
    headers = {
        ...
        "User-Agent": "teacher-whale-geography/1.0",
        "X-API-KEY": api_key,
    }
...
def download_file(url, destination):
    ...
    request = urllib.request.Request(
        url,
        headers={"User-Agent": "teacher-whale-geography/1.0"},
        method="GET",
    )
    try:
...
def main():
    ...
    try:
        ...
        generation_id, initial_response = start_generation(api_key, payload)
        ...
        result = {
            ...
            "status": final_response.get("status") or initial_response.get("status"),
            "gammaUrl": final_response.get("gammaUrl") or initial_response.get("gammaUrl"),
            "downloadUrl": final_response.get("exportUrl") or final_response.get("downloadUrl"),
        }
        ...
        if args.download_to and result.get("downloadUrl"):
            saved = download_file(result["downloadUrl"], args.download_to)
            result["savedExport"] = str(saved)
